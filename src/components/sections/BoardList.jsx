      ...
      <div className={styles['board-content']}>
        <ul className={[styles['board-list'], 'grid-title'].join(' ')}>
          <li className={styles['board-item']}>
            ...
            <span className={styles['board-conf']}>학술대회명</span>
            <span className={styles['board-title']}>논문명</span>
            <span className={styles['board-date']}>날짜</span>
            <span className={styles['board-tag']}>비고(국내/해외)</span>
          </li>
        </ul>
        <ul className={styles['board-list']}>
          {data.map((item) => (
            <li key={item.brd_idx} className={styles['board-item']}>
              ...
              <span className={styles['board-conf']}>{item.brd_ext2}</span>
              <span className={styles['board-title']}>{item.brd_title}</span>
              <span className={styles['board-date']}>
              ...
              <span className={styles['board-tag']}>{item.brd_ext1}</span>
            </li>
          ))}
        </ul>
      </div>
